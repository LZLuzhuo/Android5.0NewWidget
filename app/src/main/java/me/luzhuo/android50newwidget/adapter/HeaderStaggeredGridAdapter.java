...
public class HeaderStaggeredGridAdapter extends RecyclerView.Adapter<ViewHolder> {
	private final int TYPE_HEADER = 0x001, TYPE_ITEM = 0x002;
	private Context context;
	private int[] imageContents;

	public HeaderStaggeredGridAdapter(Context context, int[] imageContents) {
	...
	public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		...
		switch (viewType){
			case TYPE_HEADER:
				...
				break;
			case TYPE_ITEM:
				view = LayoutInflater.from(context).inflate(R.layout.item_staggered, parent, false);
				break;
		}
	...
	class RecyclerHolder extends ViewHolder {
		...
		public RecyclerHolder(View itemView) {
			...
			header_relative = itemView.findViewById(R.id.item_relative);
		...
		public void setData(int position) {
			...
			switch (this.getItemViewType()){
				case TYPE_HEADER:
					// StaggeredGrid 在这里设置, 设置最外层的布局属性为 占满列数
					StaggeredGridLayoutManager.LayoutParams slp = (StaggeredGridLayoutManager.LayoutParams) header_relative.getLayoutParams();
					...
					break;
				case TYPE_ITEM:
					// 加载网络图片, 需设置布局大小, 防止跳屏 闪屏
					/*ViewGroup.LayoutParams lp2 = item_relative.getLayoutParams();
					lp2.width = width / 2;
					lp2.height = (int) (width / 2 / 0.75);
					item_relative.setLayoutParams(lp2);*/
